...
iMSCP::Debug->newDebug('imscp-set-gui-permissions.log');
...
END{
	...
	if($main::imscpConfig{LOG_DIR} && -d $main::imscpConfig{LOG_DIR}){
		open(F, '>', "$main::imscpConfig{'LOG_DIR'}/imscp-set-gui-permissions.log") or fatal("Error: Can't open file '$main::imscpConfig{'LOG_DIR'}/imscp-set-gui-permissions.log' for writing: $!");
		print F $message;
		close F;
	}
}
