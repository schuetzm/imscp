...
sub install{
	...
	$self->fastcgiConf() and return 1;
	$self->phpConf() and return 1;
	$self->vHostConf() and return 1;
	$self->masterHost() and return 1;
	$self->installLogrotate() and return 1;
	$self->saveConf() and return 1;
	$self->setGuiPermissions() and return 1;
...
sub vHostConf {
	...
	if(-f "$self::apacheConfig{'APACHE_SITES_DIR'}/imscp.conf") {
		iMSCP::File->new(
			filename => "$self::apacheConfig{'APACHE_SITES_DIR'}/imscp.conf"
		)->copyFile("$self->{bkpDir}/imscp.conf.". time) and return 1;
	}
	...
	$cfgTpl = $httpd->buildConfFile(
		"$self->{cfgDir}/httpd.conf",
		{
			destination => "$self->{wrkDir}/imscp.conf"
		}
	);

	## Installing the new file in production directory
	my $file = iMSCP::File->new(filename => "$self->{wrkDir}/imscp.conf");
	...
	$rs = $httpd->enableSite("imscp.conf");
	return $rs if $rs;
