...
* Improved by http://www.Free-PHP-Scripts.net
*/


/*
This class must be called for each domain name,
*/

class domain{
	var $domain="";
	/*
	List of Whois servers, in format: TLD, WHOIS Server, NO Match results string
	You can Edit/add/remove servers if you want
	*/
	var $servers=array(
	...
	*/
	...
	/*
	Load domain name, if it has http:// or www. remove them
	because the funtions sees it invalid
	*/
	function domain($str_domainname){
		$erase = array('www.','http://');
		$str_domainname = str_replace($erase,'',$str_domainname);
		$this->domain=$str_domainname;
	}

	/*
	Returns the Whois information in Plain Text Format
	New lines feeds are convreted to <br/> tags
	*/
	function info(){
		if($this->is_valid()){
	...
	/*
	Return HTML formatted Whois Info
	*/
	function html_info($is_captcha){
		$text_reg = $this->info();
		//Format Emails as Images (uses Captch to encode emails)
		if($is_captcha == 1){
			$html_text = preg_replace('#([_a-zA-Z0-9]*)\@(.*)\.(.*)#',"<img src='sec_image.php?tg=$1@$2.$3'><br/>",$text_reg);
		} else {
			$html_text = preg_replace('#([_a-zA-Z0-9]*)\@(.*)\.(.*)[<^]#',"<a href='mailto:$1@$2.$3'>Email: $1@$2.$3</a><",$text_reg);
		}
		//Format URLS as Clickable Links
		$html_text = preg_replace('=([^\s]*)(www.)([^\s]*)[<^]=','<a href="http://\\2\\3" target=\'_blank\'>\\2\\3</a><',$html_text);
		return $html_text;
	}
	...
	*/
	function is_valid(){
		...
		if(count($domainArr) == 1){ return false;}
