...
function gen_client_mainmenu($tpl, $menu_file)
{
	...
	if ($user->user_alias_limit == -1 && $user->user_subd_limit == -1 && $user->user_dns != 'yes')
		$tpl->assign('ISACTIVE_DOMAIN', '');

	if ($user->user_ftpacc_limit == -1)
		$tpl->assign('ISACTIVE_FTP', '');

	if ($user->user_sqld_limit == -1)
...
function gen_client_menu($tpl, $menu_file)
{
	...
	if ($user->user_subd_limit != 0 && $sub_cnt >= $user->user_subd_limit) {
		$tpl->assign('ISACTIVE_SUBDOMAIN_MENU', '');
	}

	$als_cnt = get_user_running_als_cnt();

	if ($user->user_alias_limit != 0 && $als_cnt >= $user->user_alias_limit) {
	...
	if ($user->user_software_allowed == 'yes' && $user->user_ftpacc_limit != '-1') {
