...
function get_email_tpl_data($userId, $tplName){
	...
	$user = iMSCP_Props_user_user::getInstanceById($userId);


	if (
		trim($user->user_fname) != '' && trim($user->user_lname) != ''
	) {
		$data['sender_name'] = $user->user_fname . ' ' . $user->user_lname;
	} else if (trim($user->user_fname) != '') {
		$data['sender_name'] = $user->user_fname;
	} else if (trim($user->user_lname) != '') {
		$data['sender_name'] = $user->user_lname;
	} else {
		$data['sender_name'] = '';
	}
	...
	if ($user->user_firm != '') {
		if ($data['sender_name'] != '') {
			$data['sender_name'] .= ' ' . '[' . $user->user_firm . ']';
		} else {
			$data['sender_name'] = $user->user_firm;
		}
	}

	$data['sender_email'] = $user->user_email;
